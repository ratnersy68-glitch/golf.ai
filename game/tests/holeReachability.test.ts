...
/**
 * Every recreated real hole must be finishable as mini-golf by a (greedy) searching player within par + 2,
...
describe.each(HOLE_LAYOUTS.map((l) => [l.id, l] as const))('%s', (_id, layout) => {
  it('is playable', () => {
    ...
    // the beam search is greedy, so allow one stroke of slack over a skilled player's par + 1
    expect(r.bestStrokes).toBeLessThanOrEqual(layout.par + 2);
  }, 600000);
});
